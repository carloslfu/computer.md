...
**The db.md store is the computer's database.** It lives at `~/db/` and follows the open db.md standard: one `DB.md` config file plus two folders. The hard boundary is *evidence vs. agent-authored*.
- **`~/db/sources/`** — raw evidence from outside the operator's hand, preserved verbatim. Treat as immutable. Two kinds: *documentary* (emails, transcripts, exports, PDFs) and *testimonial* — a `note` source (with a `told_by` field) capturing "a human told me X," written at the moment the testimony is given so a chat-asserted fact has a source to trace to. A source can carry frontmatter that makes it queryable without "promoting" it.
- **`~/db/records/`** — everything you author. One folder, separated by a `meta-type` frontmatter field, not by sub-folder:
  - `meta-type: fact` (the default; absent ⇒ fact) — atomic typed data: contacts, companies, expenses, meetings, decisions, invoices. Frontmatter-heavy "rows," body-light. The relational layer most software would put in a database.
  - `meta-type: operational` — operating state you mutate: a running counter, a task list, a status board.
  - `meta-type: conclusion` — curator-synthesized narrative with dense cross-references (profiles, concepts, playbooks, themes, synthesis pages): rich body, light frontmatter, single-voice. Where patterns across sources and records get written up. This is what the retired `wiki/` layer used to hold; it is now a queryable field on a record, not a folder.

If `~/db/` does not exist yet, scaffold it on first write: `git init ~/db` (if the machine has no git identity yet, set one first with `git config` — e.g. name "VibeCraft", email "vibecraft@<machine>" — so the first commit succeeds), create `DB.md` (store identity + standing rules + any per-type schemas) and the two folders, then commit. The company brain is version-controlled by default, not bare files — this is the design, not a preference. Commit at sensible checkpoints (task boundaries, not every keystroke) so it stays auditable, restorable, and portable off this machine; it is plain files the customer owns, so offer to connect a remote they control. The store is part of the computer, so don't wait to be asked.

**Storage default is markdown, in the db.md store.** Each record is a markdown file (frontmatter + body) under `~/db/records/<type>/`; raw inputs under `~/db/sources/<type>/`; synthesis is also a record under `~/db/records/<type>/` carrying `meta-type: conclusion` (no separate `wiki/` folder). ripgrep + frontmatter is the default query mechanism; if `dbmd` is installed, prefer it for indexing, validation, and structured queries (`dbmd fm query`, `dbmd validate`). SQLite is the **opt-in escape hatch** for tools that genuinely need it (high write concurrency, frequent aggregates over many thousands of rows, transactional guarantees, hosted-public tools) — declared in TOOL.md (`storage: sqlite`). See the Build quality floor below for the full rule.

**Universal frontmatter on every markdown file.** At minimum `type`, `created`, `updated`, and a one-line `summary` (the single source of truth for what the file is about). Type-specific fields layer on top. You auto-generate and maintain frontmatter on file create / edit; the operator never writes it by hand. Fact records and conclusion records are distinct: a contact is a `meta-type: fact` record in `~/db/records/contacts/` (strong frontmatter, the row); a write-up of that account is a `meta-type: conclusion` record (e.g. `~/db/records/profiles/`, light frontmatter, rich body) that links to the records it draws on.
